...
// }
//import { fromFeature } from "./Leaflet.GeotagPhoto.FromFeature";



L.Control.cameraControl = function(geotagPhotoCamera, options) {
  return new L.Control.CameraControl(geotagPhotoCamera, options);
};

// Define class instead of new element
L.FeatureGroup.cameraCaptchure = function(feature, data, options, events) {
  return new L.FeatureGroup.CameraCaptchure(feature, data, options, events);
};

L.FeatureGroup.CameraCaptchure = L.FeatureGroup.extend({
  ...
  initialize: function(feature, data, options, events) {
    this._events = events;
    this._data = data;
  ...
  _createLayers: function() {
    ...
    this._polygon = L.polygon(
      pointList,
      Object.assign(this.options.fillStyle, {
        className: "field-of-view"
      })
    );

    this._control = L.Control.cameraControl(this, {
      ...
      contextmenuItems: [
        {
          text: "Overview",
          index: 0,
          callback: () => self._openPopupInfo(cameraLatLng)
        },
        {
          text: "View camera",
          index: 1,
          callback: () =>
            self._events &&
            self._events["viewcamera"] &&
            self._events["viewcamera"](
              _.extend(angular.copy(self._data.points), {
                id: self._data.points.pointid
              }),
              "viewcamera"
            )
        }
  ...
  _addRotateTransform: function(element, rotation) {
    ...
    if (transform.indexOf("rotate") !== -1) {
      element.style[L.DomUtil.TRANSFORM] = transform.replace(
        /rotate\(.*?\)/,
        rotate
      );
    } else {
  ...
  _updateMarkerBearings: function(fieldOfView) {
    ...
    this._addRotateTransform(
      this._angleMarker._icon,
      bearing + angle / 2 + "deg"
    );
  },
  ...
  _updateFieldOfView: function() {
    ...
    var angleLatLng = this._getAngleFromPointList(
      this._getPointList(this._fieldOfView)
    );
  ...
  _openPopupInfo: function(latLng) {
    ...
    _.forOwn(self._data.points, (value, key) => createRow(key, value));

    // Create rows with optional fields
    _.forEach(self._data.points[L.NETWORK.DATAFIELDS.OPTIONAL_FIELDS], f => {
  ...
  setAngle: function(angle) {
    this._angle = Math.max(
      Math.min(angle, this.options.maxAngle),
  ...
  setDraggable: function(draggable) {
    ...
    this._setMarkerVisible(
      this._angleMarker,
      draggable && this.options.angleMarker
    );
  },
  hiddenFOV: function() {
  ...
  showFOV: function() {
    this.setStyle({ color: "black", fillColor: "#3388ff" });
  ...
  toggleFOV: function() {
    this.getHiddenFOVStatus() ? this.showFOV() : this.hiddenFOV();
  },
  setFocusEffect: function() {
    this._cameraMarker._icon &&
      this._cameraMarker._icon.classList.add("focus-effect-point");
    setTimeout(() => {
      this._cameraMarker._icon &&
        this._cameraMarker._icon.classList.remove("focus-effect-point");
    }, 3000);
  }
});
