L.Control.ManageCamera = L.Control.extend({
  ...
  options: {
    ...
    //position: 'bottomleft',
    position: 'topleft',
    textErr: '<i class="fa fa-warning"></i> Location not found',	//error message
  ...
  onAdd: function (map) {
    ...
    this._inputWrapper = this._createInputWrapper();
    this._input = this._createInput(this.options.textPlaceholder, 'search-input', this._inputWrapper);
  ...
  _createInputWrapper: function (text, className) {
    var div = L.DomUtil.create('div', 'search-input-wrapper', this._container);
  ...
  _createInput: function (text, className, container) {
    ...
    var label = L.DomUtil.create('label', className, container);
  ...
  _createCancel: function (title, className) {
    ...
    cancel.innerHTML = "<i class=\"fa fa-times\"></i>";//imageless(see css)
  ...
  _createButton: function (title, className) {
    ...
    button.href = '#';
    button.title = title;
    button.innerHTML = "<i class=\"fa fa-search\"></i>";//imageless(see css)
  ...
  showTooltip: function (records) {
    ...
    if (this._countertips > 0) {
      ...
      var noResultElem = L.DomUtil.create('li', '');
      noResultElem.innerHTML = 'Not found';
      L.DomUtil.addClass(noResultElem, 'search-tip');
      this._tooltip.appendChild(noResultElem);
      //this._hideTooltip();
    }
  ...
  _formatData: function(rawJson) {
    let json = {};

    _.forOwn(rawJson, (o)=> {
      let key = o.formatted_address;
      let loc = L.latLng(o.geometry.location.lat(), o.geometry.location.lng());
...
L.Control.ManageCamera.Marker = L.Marker.extend({
...
L.control.manageCamera = function (options) {
  return new L.Control.ManageCamera(options);
};
